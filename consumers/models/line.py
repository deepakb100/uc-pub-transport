...
class Line:
    ...
    def process_message(self, message):
        ...
        # Dome: Based on the message topic, call the appropriate handler.
        if "transformed-stations" == message.topic(): # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "station." in message.topic(): # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif "TURNSTILE_SUMMARY" == message.topic(): # Set the conditional to the KSQL Turnstile Summary Topic
            json_data = json.loads(message.value())
            station_id = int(json_data.get("SID"))
            station = self.stations.get(station_id)
            if station is None:
                logger.debug("unable to handle message due to missing station")
                return
